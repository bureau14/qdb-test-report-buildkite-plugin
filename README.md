...
Job and aggregate annotations separate failed and errored executions into two tables. The Boost.Test/test-runner table shows `suite::file`, test case, failure reason, and execution time. The CTest table shows test case (without the repeated class name), failure reason, and execution time. Execution time is shown in seconds to three decimal places in the final column; missing durations in older summaries appear as a dash. Neither table includes status, targets, or XML links. Errored executions have an `ERRORED:` prefix in the failure reason; ordinary failures have no prefix. Failure reasons are flattened to one line and capped at 240 characters, including the prefix; missing reasons appear as a dash. Both tables share Buildkite's 1 MiB annotation limit, with a count of omitted executions when necessary. The summary and full-report link retain their space in the annotation.
